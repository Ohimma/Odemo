...
http.put = function (url, obj, needToken = true) {
    ...
    return new Promise((resolve, reject) => {
        axios(config)
            .then(response => {
                if (response.status === 200 || response.status === 201 || response.data.id || response.result === 'completed' || response.result === 'updated') {
                    resolve(response)
                    this.$message({
                        message: 'put 更新成功',
                        type: 'success'
...
http.delete = function (url, obj, needToken = true) {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (response.status === 200 || response.data.result === "deleted" || response.data.result === "complete") {
          resolve(response)
          this.$message({
            message: 'delete 删除成功',
            type: 'success'
